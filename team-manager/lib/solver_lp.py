...
def _build_prob(players, teams, d_idx, f_idx, cougar_idx, n_players):
    ...
    team_rating = [lpSum(players[i]['rating'] * assigned[i][t] for i in range(n_all)) for t in teams]
    ...
    return prob, assigned, is_cougar_team, cougar_placed, cougar_count, max_rating, min_rating, max_size, min_size, max_def, min_def
...
def assign_teams(players):
    ...
    prob1, assigned1, is_cougar_team1, cougar_placed1, cougar_count1, *_ = \
        _build_prob(players, teams, d_idx, f_idx, cougar_idx, n_players)
    prob1 += -cougar_count1
    prob1.solve(PULP_CBC_CMD(msg=0))
    max_cougars = int(round(value(cougar_count1)))

    # Phase 2 — lock Cougar count, minimise size spread (×1000) + def spread (×500) + rating spread
    prob2, assigned2, is_cougar_team2, cougar_placed2, cougar_count2, \
        max_rating, min_rating, max_size, min_size, max_def, min_def = \
        _build_prob(players, teams, d_idx, f_idx, cougar_idx, n_players)
    prob2 += cougar_count2 >= max_cougars
    prob2 += 1000 * (max_size - min_size) + 500 * (max_def - min_def) + (max_rating - min_rating)
    ...
    if cougar_idx:
        cougar_counts = [sum(value(assigned2[i][t]) for i in cougar_idx) for t in teams]
